<template>
  <section :id="sectionId" class="planets container">
    <h2 class="planets__title">Планеты</h2>
    <p class="planets__subtitle">Как распределены акценты по планетам вашей карты.</p>

    <template v-if="!hasPlanets">
      <p class="planets__placeholder">Данные по планетам недоступны. Попробуйте позже.</p>
    </template>
    <template v-else>
      <div class="planets__filters">
        <FilterChips
          v-model="sortBy"
          class="planets__filter"
          label="Сортировка"
          aria-label="Сортировка планет"
          name="planets-sort"
          :options="sortOptions"
        />
        <FilterChips
      ...
      <div class="planets__grid">
        ...
        >
          ...
          <p class="planet-card__position">в {{ vm.signLabel }}, {{ vm.houseLabel }}</p>
          <div class="planet-card__importance">
            ...
            <span class="planet-card__importance-value">
              {{ importanceDisplay(vm.importance) }}/10
            </span>
          </div>
          <div v-if="vm.lifeAreas.length > 0" class="planet-card__life-areas">
            <span v-for="area in vm.lifeAreas.slice(0, 3)" :key="area" class="planet-card__chip">
              {{ area }}
            </span>
          </div>
          <div v-if="vm.tags.length > 0" class="planet-card__tags">
            <span v-for="tag in vm.tags.slice(0, 3)" :key="tag" class="planet-card__tag">
              {{ tag }}
            </span>
          </div>
        </button>
                  ...
                  <template v-else>
                    ...
                    >
                      ...
                      <ul class="planets-modal__aspect-list">
                        ...
                        >
                          <span class="planets-modal__aspect-type">
                            {{ aspectTypeLabel(a.type) }}
                          </span>
                          ...
                          <p v-if="aspectDescriptionShort(a)" class="planets-modal__aspect-desc">
...
<script setup lang="ts">
    ...
    if (strongAspects.length === 0) {
      for (const p of planets) {
        ...
        const planetAspects = aspects.filter(
          (a) =>
            (a.planet1 === p.name || a.planet2 === p.name) && (a.orb ?? 99) <= STRONG_ORB_THRESHOLD
        );
  ...
  function buildPlanetViewModels(data: AstroApiResponse): PlanetViewModel[] {
    ...
    return planets.map((p) => {
      ...
      const planetAspects = aspects.filter((a) => a.planet1 === p.name || a.planet2 === p.name);
      ...
      const planetTagKeys = new Set([
        ...Object.keys(PLANET_LABELS),
        ...Object.values(PLANET_LABELS).map((label) => label.toLowerCase()),
      ]);
      const allTags = [...new Set([...signTags, ...houseTags])]
        .filter(Boolean)
        .filter((tag) => !planetTagKeys.has(tag.toLowerCase()))
        .slice(0, 5);

      const lifeAreaKeys = lifeAreaMap.get(p.name) ?? [];
      const lifeAreas = lifeAreaKeys.map((k) => LIFE_AREA_LABELS[k] ?? k).filter(Boolean);
  ...
  const sortOptions = [
    ...
    { value: 'house', label: 'По домам' },
  ];

  const lifeAreaOptions = computed(() => [
    { value: '', label: 'Все сферы' },
    ...Object.entries(LIFE_AREA_LABELS).map(([value, label]) => ({ value, label })),
  ]);

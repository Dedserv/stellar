<template>
  <section :id="sectionId" class="insights container">
    ...
    <div v-else class="insights__grid">
      <article v-for="insight in topInsights" :key="insight.id" class="insight-card">
        ...
        <button type="button" class="insight-card__btn" @click="toggleExpanded(insight.id)">
...
<script setup lang="ts">
  import type { ChartResponse, Insight, InsightEvidence } from '~/types/natal';
  ...
  const topInsights = computed<Insight[]>(() => {
    ...
    return aspects.sort((a, b) => b.score - a.score).slice(0, 6);
  });
